...
import { Dialog, DialogContent, DialogActions, DialogTitle, Grid, Input, DatePicker, Box, Button, Fab, Chip, Stack, Loader, TableContainer, Table, TableBody, TableHead, TableRow, TableCell, Paper, Avatar, IconButton, Menu, MenuItem, Typography, TableFooter, FormControl, FormLabel, Select } from "components";
import { addIncome, updateIncome, deleteTransaction } from "services/firebase";
import { useGetAllFieldQuery, useGetAllIncomeQuery } from "services/queries";
...
function Home() {
    ...
    const { data, refetch } = useGetAllIncomeQuery(user);
    ...
    return (
        <Layout>
            <Box>
                <Typography variant="h3" sx={{ mb: "30px" }}>Incomes</Typography>
                {pageData && fieldData ? (
                    <>
                        <Box>
